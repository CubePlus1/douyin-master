    ...
    <div class="slide-imgs">
      <SlideHorizontal v-model:index="state.index">
        <SlideItem v-for="(item, i) in state.detail.imgs" :key="i">
          <img v-lazy="getGoodsImg(item)" alt="" />
        </SlideItem>
      </SlideHorizontal>
...
function getGoodsImg(item) {
  if (!item) return _checkImgUrl('goods/placeholder.png')
  if (item.includes('assets/img') || item.includes('file://') || item.includes('http')) {
    return _checkImgUrl(item)
  }
  return _checkImgUrl('goods/' + item)
}
  ...
  .content {
    ...
    .comments {
      .debate-entry {
        ...
        border-radius: 12rem;
        color: white;
        background: linear-gradient(90deg, rgb(248, 38, 74), #ff7a45);
