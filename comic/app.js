/* ==========================================================================
   🎮 CORE LOGIC: COSMETICS BATTLE ROYALE AVG (app.js)
   ========================================================================== */
...
class BleepSynth {
  ...
  playVictoryFanfare() {
    try {
      ...
      // Triumphant quick arpeggio arpeggio C4-E4-G4-C5 and sustain C5 chord
...
function addLog(message, type = 'system') {
  ...
  if (!container) return; // Prevent crashes if element is missing
...
const productRecData = {
  ...
  science: {
    ...
    reasons: [
      ...
      { title: "温和修护融肤度高", desc: "高度贴合肌肤纹理，自然透光，修饰面部毛孔的同时维持高水准的融肤舒适感。" }
    ],
    risks: [
...
function showAVGResults(productType) {
  ...
  el.resProductName.innerText = data.name;
  el.resPriceRange.innerText = data.price;
  el.resVerdict.innerText = data.verdict;
  el.resScoreText.innerText = `${data.score}%`;
...
function executeAVGStep() {
  ...
  if (speaker === 'system') {
    // System message
    AppState.elements.speakerRoleLabel.innerText = '广播公证';
    ...
    // Text and Audio
    AppState.elements.dialogueText.innerText = step.text;
    setDecibelValue(35);
    ...
    AppState.elements.speakerRoleLabel.innerText = '消费者（混油通勤党）';
    ...
    const pmInfo = AppState.charData[speaker];
...
// Top Alert Panic (jump directly to climax victory at index 10)
function triggerBossPanic() {
  if (AppState.isFinished) return;
  
  // Set index to index 10 ("所以结论更清楚了...")
  AppState.scriptIndex = 10;
  executeAVGStep();
}
...
function bindEvents() {
  ...
  // 1. Audio Activation Enter Modal
  el.btnEnter.addEventListener('click', () => {
    synth.init();
    el.audioModal.classList.add('hide-modal');
    addLog('>> 玩家进入了战场。音效合成器已激活。开始互动！', 'system');
    
    // Automatically trigger step 0 of AVG
    setTimeout(() => {
      resetAllShowStats();
    }, 300);
  });

  // 2. Click anywhere on the entire webpage document to advance (AVG standard experience)
  document.addEventListener('click', (e) => {
    ...
    // If clicking modal enter button, ignore
    if (e.target.closest('#btn-enter')) return;
    // If clicking anywhere inside the results card, ignore to prevent narrative advancing
    if (e.target.closest('#results-card')) return;
    
    // Ignore clicks if the audio modal is still showing
    if (!el.audioModal.classList.contains('hide-modal')) return;

    executeAVGStep();
  });

  // 3. Click next arrow button to advance
  el.nextBtn.addEventListener('click', (e) => {
    e.stopPropagation(); // Stop event bubbling completely!
    executeAVGStep();
  });

  // 4. Header Consumer Panic Button
  el.btnBoss.addEventListener('click', (e) => {
    e.stopPropagation(); // Stop propagation to prevent advancing story index
    triggerBossPanic();
  });

  // 5. Results Card Actions & Restart Binding
  const resultsCard = document.getElementById('results-card');
  if (resultsCard) {
    resultsCard.addEventListener('click', (e) => {
      e.stopPropagation(); // Block clicks inside results-card from reaching document
    });
  }
  ...
  if (btnRestart) {
    btnRestart.addEventListener('click', (e) => {
      e.stopPropagation();
      if (resultsCard) {
        resultsCard.classList.add('hide-card');
      }
      resetAllShowStats();
    });
  }

  // Interactive mockup CTAs for remaining results buttons
  const primaryBtn = document.getElementById('btn-primary');
  if (primaryBtn) {
    primaryBtn.addEventListener('click', () => {
      alert('🛒 已为您模拟跳转至【兰蔻持妆清透粉底液】官方购买渠道！');
    });
  }
  ...
  if (traceBtn) {
    traceBtn.addEventListener('click', () => {
      alert('🔍 溯源成功！兰蔻持妆清透粉底液凭借 94 分最高持妆度、高匹配混油皮与卓越防暗沉控油力位列第一名！');
    });
  }

  // --- 🏆 AVG Interactive Finished Card Clicks ---
  const bindCardClick = (pmType) => {
    const cardEl = pmType === 'luxury' ? AppState.elements.cardLuxury :
                   pmType === 'science' ? AppState.elements.cardScience :
                   AppState.elements.cardBudget;
    
    if (cardEl) {
      cardEl.addEventListener('click', (e) => {
        e.stopPropagation(); // Stop click from propagating to document (which advances AVG step)
        
        // If the AVG is finished (climax victory reached):
        if (AppState.isFinished) {
          // Play matching speech audio blip
          synth.playSpeak(pmType);
          
          // Update top cards visual states:
          // Set clicked card to active/idle state (ungreyed) and others to greyed!
          const pms = ['luxury', 'science', 'budget'];
          pms.forEach(pm => {
            if (pm === pmType) {
              setCardState(pm, 'idle');
  ...
  const btnRestartResults = AppState.elements.btnRestartResults;
